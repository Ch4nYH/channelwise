...
class SimpleModel(nn.Module):
    ...
    def get_channel_num(self):
        ...
        for i in range(len(self.layers)):
            n[self.layers[i]] = getattr(self, self.layers[i]).weight.shape[0]
        
        return n
    
    def get_channel_names(self):
        num_dict = self.get_channel_num()
        name_list = []
        for name in num_dict:
            for j in range(num_dict[name]):
    ...
    def get_channel_stats(self):
        s = defaultdict()
        n = self.get_channel_num()
        for i in range(len(self.layers)):
            s[self.layers[i]] = torch.zeros((n[self.layers[i]], 2))
            for j in range(n[self.layers[i]]):
                s[self.layers[i]][j, 0] = torch.mean(getattr(self, self.layers[i]).weight[j, ...])
                s[self.layers[i]][j, 1] = torch.std(getattr(self, self.layers[i]).weight[j, ...])
        return s
